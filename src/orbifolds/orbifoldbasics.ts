 ...
 */
export function applyMatrix(matrix: Matrix3x3, x: number, y: number): { x: number; y: number } {
 ...
 * y = r * sqrt(3)/2
 */
export function axialToCartesian(q: number, r: number): { x: number; y: number } {
  return {
    x: q + r * 0.5,
    y: r * Math.sqrt(3) / 2,
  };
...
/**
 * Format a voltage matrix for display as a single line.
 * Example output: "[[1,0,0], [0,1,0], [0,0,1]]"
 */
export function formatVoltage(v: Matrix3x3): string {
  return `[[${v[0].join(",")}], [${v[1].join(",")}], [${v[2].join(",")}]]`;
}

/**
 * Format a voltage matrix for display as multiple lines (for UI).
 * Returns an array of 3 strings, one per row.
 */
export function formatVoltageRows(v: Matrix3x3): string[] {
  return [
    `[${v[0].join(", ")}]`,
    `[${v[1].join(", ")}]`,
    `[${v[2].join(", ")}]`,
 ...
 */
export function getLiftedNodeAbsolutePosition<ND extends ExtraData = ExtraData>(
  orbifoldGrid: OrbifoldGrid<ND>,
